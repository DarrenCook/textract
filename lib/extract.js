var fs = require( 'fs' )
  ...
  , WHITELIST_PRESERVE_LINEBREAKS = /[^A-Za-z\x80-\xFF\x24\u20AC\xA3\xA5 0-9 \u2015\u2116\u2018\u2019\u201C|\u201D\u2026 \uFF0C \u2013 \u2014 \u00C0-\u1FFF \u2460-\uFFFF \.,\?""!@#\$%\^&\*\(\)-_=\+;:<>\/\\\|\}\{\[\]`~'-\w\n\r]*/g  // eslint-disable-line max-len
  , WHITELIST_STRIP_LINEBREAKS = /[^A-Za-z\x80-\xFF\x24\u20AC\xA3\xA5 0-9 \u2015\u2116\u2018\u2019\u201C|\u201D\u2026 \uFF0C \u2013 \u2014 \u00C0-\u1FFF \u2460-\uFFFF \.,\?""!@#\$%\^&\*\(\)-_=\+;:<>\/\\\|\}\{\[\]`~'-\w]*/g  // eslint-disable-line max-len
  ;
...
function cleanseText( options, cb ) {
  return function( error, text ) {
    if ( !error ) {
      ...
      //See https://github.com/dbashford/textract/issues/151 for what
      //we are doing here. NB. Asian languages only!!!
      text = text.replace( / +(?! )/g, '' )
        .replace( /[\t\v\u00A0]{2,}/g, ' ' );
      //Remove single LFs, but keep double LFs
      //  (it appears this can equally well go higher up this function)
      text = text.replace(/([^\n])\n([^\n])/g,'$1$2')
